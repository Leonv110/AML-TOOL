...
def process_aml_rules():
    ...
    if 'is_flagged' not in df.columns:
        df['is_flagged'] = False
    if 'flagged_reason' not in df.columns:
        df['flagged_reason'] = ""
    ...
    if 'country_risk_level' in df.columns:
        ...
        for idx, row in df.iterrows():
            if str(row['country_risk_level']).strip().lower() == 'high':
                ...
                df.at[idx, 'flagged_reason'] = append_reason(df.at[idx, 'flagged_reason'], 'Geo-Risk')

    # 2. Dormancy: Flag if days_since_last_transaction > 90.
    if 'days_since_last_transaction' in df.columns:
        print("Applying Dormancy rule...")
        for idx, row in df.iterrows():
            try:
                days_since_last = float(row['days_since_last_transaction'])
                if pd.notna(days_since_last) and days_since_last > 30: # Lowered threshold to 30 days
                    df.at[idx, 'is_flagged'] = True
                    df.at[idx, 'flagged_reason'] = append_reason(df.at[idx, 'flagged_reason'], 'Dormancy')
            except (ValueError, TypeError):
                pass # safely ignore rows with invalid days_since_last_transaction

    # 3. Structuring: Use a 7-day rolling window to flag if a user's total transactions exceed 200,000 but individual ones are smaller.
    if all(col in df.columns for col in ['user_id', 'timestamp', 'amount']):
        print("Applying Structuring rule...")
        df['Date_Temp'] = pd.to_datetime(df['timestamp'], errors='coerce')
            ...
            for idx in user_data.index:
                ...
                try:
                    ...
                    if window_sum > 50000 and current_amount < 50000 and transaction_count >= 3: # Lowered threshold to $50k
                        df.at[idx, 'is_flagged'] = True
                        df.at[idx, 'flagged_reason'] = append_reason(df.at[idx, 'flagged_reason'], 'Structuring')
                except ValueError:
                    continue
    ...
    if 'amount' in df.columns and 'transaction_frequency_1hr' in df.columns:
        print("Applying Spike Detection rule...")
        ...
        if len(X) > 0:
            ...
            for idx, pred in zip(df.index, preds):
                if pred == -1: # Outlier detected (-1)
                    df.at[idx, 'is_flagged'] = True
                    df.at[idx, 'flagged_reason'] = append_reason(df.at[idx, 'flagged_reason'], 'Spike Detection')

    # 5. Layering: Create a directed graph with user_id and destination_id to find circular transfers.
    if 'user_id' in df.columns and 'destination_id' in df.columns:
                    ...
                    for i in range(len(cycle_list)):
                        ...
                        if edge_data and 'indices' in edge_data:
                            for idx in edge_data['indices']:
                                df.at[idx, 'is_flagged'] = True
                                df.at[idx, 'flagged_reason'] = append_reason(df.at[idx, 'flagged_reason'], 'Layering')
        except Exception as e:
            print(f"Error executing Layering detection: {e}")

    # Finalize Output: Push updates to Supabase using bulk update
    print("Preparing updates for Supabase...")
    
    # Assuming the table has an auto-incrementing id or primary key `transaction_id`
    if 'transaction_id' in df.columns:
        # Filter for only the rows that were flagged to minimize payload
        flagged_df = df[df['is_flagged'] == True]
        updates_count = len(flagged_df)
        
        if updates_count > 0:
            # Convert flagged rows to a list of dicts for the upsert/bulk update
            update_payload = flagged_df[['transaction_id', 'is_flagged', 'flagged_reason']].to_dict(orient='records')
            
            # Using upsert requires the primary key ('transaction_id') to be in the payload, 
            # which we have included. It will update existing rows.
            try:
                # Upsert updates the records if the ID exists, effectively acting as a batch update
                response = supabase.table("transactions").upsert(update_payload).execute()
                print(f"Successfully processed rules. Bulk updated {updates_count} flagged records.")
            except Exception as e:
                print(f"Failed to execute bulk update: {e}")
        else:
             print("Successfully processed rules. No new flags to update.")
    else:
        print("Notice: No 'transaction_id' primary key column found in DataFrame. Could not update Supabase.")
